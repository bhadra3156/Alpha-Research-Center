...
function fmt(v:number):string { return isNaN(v)||!v?"$0.00":"$"+v.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2}); }
...
export default function Portfolio() {
  ...
  useEffect(()=>{
    try { const s=localStorage.getItem("alpha_positions"); if(s) setPositions(JSON.parse(s)); } catch(e){}
  },[]);
  ...
  const openEdit=(p:Position)=>{
    setEditId(p.id); setTicker(p.ticker); setMarket(p.market); setEntryDate(p.entry_date);
    setEntryPrice(String(p.entry_price)); setShares(String(p.shares));
    setStopLevel(String(p.stop_level||"")); setTargetPrice(String(p.target_price||""));
    setNotes(p.notes||""); setShowForm(true); setMessage(null); setAnalysis(null);
  ...
  const handleSave=()=>{
    ...
    setSaving(true);
    const pos:Position={ id:editId||Date.now().toString(), ticker:ticker.trim().toUpperCase(), market, entry_date:entryDate,
      entry_price:n(entryPrice), shares:n(shares), stop_level:n(stopLevel), target_price:n(targetPrice), notes:notes.trim() };
  ...
  const analyzePortfolio=async()=>{
    ...
    try {
      const portfolioSummary=positions.map(p=>{
        ...
        return `${p.ticker}: ${n(p.shares)} shares @ $${n(p.entry_price).toFixed(2)} (Cost: $${cost.toLocaleString(undefined,{maximumFractionDigits:0})})${p.stop_level?` Stop: $${n(p.stop_level).toFixed(2)}`:""}${p.target_price?` Target: $${n(p.target_price).toFixed(2)}`:""}${p.notes?` Notes: ${p.notes}`:""}`;
      }).join("\n");
...
Be direct, institutional, and concise. No disclaimers.`}]
  ...
  const generateLocalAnalysis=(positions:Position[]):string=>{
...
SWOT ANALYSIS
━━━━━━━━━━━━
STRENGTHS: Diversified across ${positions.length} positions. Active position management with defined entries.
WEAKNESSES: ${largestPct}% concentration in ${largest.ticker} — largest single position. ${positions.filter(p=>!p.stop_level).length} positions without stop losses defined.
OPPORTUNITIES: Review positions without target prices set — define exit strategies to lock in gains.
THREATS: No stop losses on ${positions.filter(p=>!p.stop_level).length} positions creates unlimited downside risk.
...
${positions.map(p=>{
  ...
  const verdict=cost/total>0.4?"REVIEW SIZE":!p.stop_level?"SET STOP LOSS":"HOLD";
  return `${p.ticker}: ${verdict} — ${pct}% of portfolio ($${cost.toLocaleString(undefined,{maximumFractionDigits:0})})`;
}).join("\n")}

TOP RECOMMENDATION
━━━━━━━━━━━━━━━━
Set stop losses on all positions immediately. Define maximum loss per position at 7-8% below entry. This single action converts speculation into professional risk management.
      ...
      <div style={{maxWidth:"1200px",margin:"0 auto",padding:"72px 20px 40px"}}>
        ...
        {analyzing&&(
          <div style={{background:"linear-gradient(145deg,#0f172a,#1e293b)",border:"1px solid rgba(96,165,250,0.3)",borderRadius:"14px",padding:"30px",marginBottom:"16px",textAlign:"center"}}>
            ...
            <div style={{color:"#475569",fontSize:"12px"}}>Senior hedge fund PM reviewing positions · SWOT analysis · Hold/Buy/Sell verdicts</div>
          </div>
        )}
