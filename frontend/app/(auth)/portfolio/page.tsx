...
function fmt(v:number):string{return v>0?"$"+v.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2}):"â€”";}
    ...
    <>
      <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <KpiCard label="Markets" value={positions.length>0?[...new Set(positions.map(p=>p.market))].join(" / "):"â€”"} sub="US / UK exposure"/>
        </div>
        {/* ACTION BAR */}
        <div className="flex items-center gap-3">
          {positions.length>0&&<button onClick={analyzePortfolio} disabled={analyzing} className={`px-4 py-2 text-xs font-medium rounded border transition-colors ${analyzing?"text-[#52525b] border-[#27272a]":"bg-blue-500/10 text-blue-400 border-blue-500/30 hover:bg-blue-500/20"}`}>{analyzing?"ðŸ§  Analyzing...":"ðŸ§  AI Analysis"}</button>}
        ...
        {analyzing&&<div className="bg-[#18181b] border border-[#27272a] rounded-xl p-10 text-center space-y-2"><p className="text-blue-400 text-sm font-semibold">ðŸ§  Analyzing Portfolio...</p><p className="text-xs text-[#52525b]">Senior hedge fund PM Â· Web search Â· SWOT Â· Position verdicts</p></div>}
        {analysis&&!analyzing&&(
          <div className="w-full overflow-hidden rounded-lg border border-[#1E2530] bg-[#0C0F16]">
            ...
            <div className="px-4 py-2 border-t border-[#1E2530] bg-[#0A0D14] text-[10px] text-[#3f3f46]">Powered by Claude AI Â· Not financial advice</div>
          </div>
        )}
        {/* TABLE */}
        <div className="w-full overflow-hidden rounded-lg border border-[#1E2530] bg-[#0C0F16]">
            ...
            <div className="overflow-x-auto"><table className="w-full">
              ...
              <tbody>
                {positions.map((p,i)=>{
                  ...
                  const rr=sl>0&&tp>0?((tp-ep)/(ep-sl)).toFixed(1):"â€”";
                  return(
                    <tr key={p.id} className="border-b border-[#1E2530]/50 last:border-0 hover:bg-[#161C28]/60 transition-colors">
                      ...
                      <td className="px-4 py-2.5 text-xs text-[#52525b]">{p.market==="US"?"US":"ðŸ‡¬ðŸ‡§"}</td>
                      ...
                      <td className={`px-4 py-2.5 font-mono text-xs ${sl>0?"text-rose-400":"text-[#27272a]"}`}>{sl>0?fmt(sl):"â€”"}</td>
                      <td className={`px-4 py-2.5 font-mono text-xs ${tp>0?"text-emerald-400":"text-[#27272a]"}`}>{tp>0?fmt(tp):"â€”"}</td>
                      <td className={`px-4 py-2.5 font-mono text-xs ${rr!=="â€”"&&parseFloat(rr)>=2?"text-emerald-400":rr!=="â€”"?"text-amber-400":"text-[#27272a]"}`}>{rr!=="â€”"?rr+"x":"â€”"}</td>
                      <td className="px-4 py-2.5 text-xs text-[#52525b] max-w-[120px] truncate">{p.notes||"â€”"}</td>
                      <td className="px-4 py-2.5"><div className="flex gap-1.5"><button onClick={()=>openEdit(p)} className="text-[10px] px-2 py-1 rounded border bg-blue-950/40 text-blue-400 border-blue-800/30 hover:bg-blue-950/70 transition-colors">Edit</button><button onClick={()=>remove(p.id)} className="text-[10px] px-2 py-1 rounded border bg-rose-950/40 text-rose-400 border-rose-800/30 hover:bg-rose-950/70 transition-colors">âœ•</button></div></td>
                    </tr>
                  );
                })}
              </tbody>
